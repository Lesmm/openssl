...
#
#
# Reference: https://github.com/x2on/OpenSSL-for-iPhone/blob/1.0.2i/build-libssl.sh
#
#
# curl -O https://www.openssl.org/source/openssl-1.0.0a.tar.gz
# curl https://codeload.github.com/openssl/openssl/zip/refs/tags/OpenSSL_1_0_2k -o OpenSSL_1_0_2k.zip
# curl https://codeload.github.com/openssl/openssl/tar.gz/refs/tags/OpenSSL_1_0_2k -o OpenSSL_1_0_2k.tar.gz
# curl -O https://github.com/openssl/openssl/archive/OpenSSL_1_0_2k.tar.gz
#
...
if [[ -z "${BRANCH}" ]]; then
  ...
  BRANCH=$(git rev-parse --abbrev-ref HEAD | sed -n "s/^.*_\([0-9]*_[0-9]*_[0-9].*\)$/\1/p")
fi
...
echo "  Script directory and build location: ${BUILD_DIR}"
echo
...
if [ "${CLEANUP}" == "true" ]; then
  ...
  if [ -d "${BUILD_DIR}/include/openssl" ]; then
    rm -r "${BUILD_DIR}/include/openssl"
  fi
...
for ARCH in ${ARCHS}
do
  # Do a absolutely Clean the libcrypto.a for the ar errors: https://stackoverflow.com/q/37063202
  rm .gitignore && git clean -f --quiet && git checkout .gitignore
  ...
  # Prepare target dir
  TARGETDIR="${BUILD_DIR}/bin/${PLATFORM}${SDKVERSION}-${ARCH}.sdk"
  mkdir -p "${TARGETDIR}"
  ...
  # Prepare source dir
  # SOURCEDIR="${BUILD_DIR}/src/${PLATFORM}-${ARCH}"
  # mkdir -p "${SOURCEDIR}"
  # tar zxf "${BUILD_DIR}/${OPENSSL_ARCHIVE_FILE_NAME}" -C "${SOURCEDIR}"
...
if [ ${#LIBSSL_IOS} -gt 0 ]; then
  ...
  lipo -create ${LIBSSL_IOS[@]} -output "${BUILD_DIR}/lib/libssl.a"
  lipo -create ${LIBCRYPTO_IOS[@]} -output "${BUILD_DIR}/lib/libcrypto.a"
fi

# Build tvOS library if selected for build
if [ ${#LIBSSL_TVOS} -gt 0 ] ; then
    echo "Build library for tvOS..."
    lipo -create ${LIBSSL_TVOS[@]} -output "${BUILD_DIR}/lib/libssl-tvOS.a"
    lipo -create ${LIBCRYPTO_TVOS[@]} -output "${BUILD_DIR}/lib/libcrypto-tvOS.a"
fi

# Copy include directory
cp -R "${INCLUDE_DIR}" ${BUILD_DIR}/include/

echo "Build done in ${BUILD_DIR}"
echo "Done."
